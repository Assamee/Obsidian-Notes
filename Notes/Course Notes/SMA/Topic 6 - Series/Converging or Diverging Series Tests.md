...
| :------------------------------------------------------------------------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| $\text{Factorials } k! \text{ or } \text{Exponentials } (3^k)$                              | $\text{Ratio Test}$                       | $\text{Ratio Test cancels out factorials perfectly}$<br>$\left( \text{e.g. } \frac{(n+1)!}{n!} = n +1 \right)$                                                                                             |
| $\text{Entire term raised to the } n^{th} \text{ power } (...)^k$                           | $\text{Root Test}$                        | $\text{The } n^{th} \text{ root } \sqrt[n]{a_n} \text{ removes the power}$                                                                                                                                 |
| $\text{Rational Functions (Polynomial over Polynomial)}$<br>$\text{e.g. }\frac{n}{n^3 + 1}$ | $\text{Limit Comparison (Quotient Test)}$ | $\text{The Ratio Test usually falis here (results in 1)}$<br>$\text{So we compare it to a ``similar" P-Series, based of the highest powers}$<br>$\left( \text{e.g. }\frac{k}{k^3} = \frac{1}{k^2} \right)$ |
| $\text{Logarithms }(ln(k))  \text{ or simple decreasing functions}$                         | $\text{Integral Test}$                    | $\text{Use \textbf{only} if we can easily integrate the function}$                                                                                                                                         |
| $\text{Oscillating Term} (\sin k, \cos k)$                                                  | $\text{Direct Comparison Test}$           | $\text{Limits don't exist for Oscillating terms}$<br>$\text{So we replace them with their mix/max values}$<br>$\left( \text{e.g. } \cos k \leqslant 1 \right) \text{ to build an inequality}$              |
| $\text{Alternating Signs } (-1^n)$                                                          | $\text{Alternating Sign Test}$            | $\text{This test is designed for series with negative terms}$                                                                                                                                              |

# Tests:
## $\textbf{1. The Ratio Test:}$
- $\text{Best for Factorials } (k!) \text{ and Exponentials } (3^k)$
#### $\text{Formula:}$ 
- $\text{Calculate the limits of the ``next term divided by the cuurent term"}$
